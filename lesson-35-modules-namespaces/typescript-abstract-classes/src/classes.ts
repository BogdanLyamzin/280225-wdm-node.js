...
class Product {
  ...
  private static statusList: string[] = ["inStock", "sale", "hidden"];
  private static shopName: string = "Amazon";

  static getCheapestProduct = (products: Product[]): Product => {
    return products.reduce((acum: Product, product: Product): Product => {
      if (acum.getPrice() > product.getPrice()) {
        return product;
      }
      return acum;
    });
  ...
  static calcTotalProductsPrice = (products: Product[]): number => {
    return products.reduce(
      (acum: number, product: Product): number => acum + product.getPrice(),
      0,
    );
  };
  ...
  constructor(
    ...
    this._price = price;
    if (status && Product.statusList.includes(status)) {
      this._status = status;
    }
  ...
  set status(newStatus: string) {
    if (!Product.statusList.includes(newStatus)) {
      throw new Error(`status can be only ${Product.statusList.join(" or ")}`);
    }
...
class Phone extends Product {
  ...
  setMemorySize(newMemorySize: number): void {
    this.isFiniteNumber(newMemorySize);
    if ((newMemorySize as number) < 0) {
      throw new Error("Memory size can't be less then 0");
    }
...
const phone = new Phone(
  "iPhone Air 17",
  "Самый тонкий телефон в мире",
  1000,
  256,
);
// console.log(phone.calcDelivery(100))

const cheapestProduct = Product.getCheapestProduct([product1, phone]);
